...
% \begin{frame}{Règles PEP8 -- Résumé des conventions}
% \footnotesize
% \begin{tabular}{|l|l|} 
%  \hline
%  Catégorie & Règle \\ 
%  \hline
%  Indentation & Utiliser 4 espaces pour indenter le code \\
%  \hline
%  Espaces Blancs & Ajouter une ligne vide entre les fonctions et les classes \\
%  \hline
%  Longueur des lignes & Limiter la longueur des lignes à 79 caractères \\
%  \hline
%  Noms de variables & Utiliser des noms en minuscules avec des underscores (snake\_case) \\
%  \hline
%  Noms de fonctions & Utiliser des noms en minuscules avec des underscores (snake\_case) \\
%  \hline
%  Noms de classes & Utiliser des noms en CamelCase \\
%  \hline
%  Commentaires & Utiliser des commentaires pour expliquer le code, pas simplement répéter ce qu'il fait \\
%  \hline
%  Importations & Grouper les importations par module et utiliser des importations absolues lorsque possible \\
%  \hline
%  Encodage & Pas de caractères spéciaux, utiliser l'anglais et l'encodage UTF-8\\
%  \hline
% \end{tabular}
...
\begin{frame}[fragile]{Conventions PEP8 à retenir}
  \begin{block}{{\large Longueur des lignes}}
    \medskip
  ...
  \end{block}
  \bigskip
  \begin{block}{{\large Encoding et langue}}
    \medskip
...
\begin{frame}{Conventions PEP8 à retenir}
  \begin{block}{{\large Conventions de nommage}}
    \medskip
...
\begin{frame}{Conventions PEP8 à retenir}
  \begin{block}{{\large Conventions de nommage}}
    ...
    \begin{tabular}{|l|l|} 
 \hline
  \textbf{Pour quoi} & \textbf{Type} \\ 
 \hline
  Packages & \texttt{lowercase} \\
 \hline
  Modules & \texttt{lowercase} ou \texttt{lowercase\_with\_underscores} \\
  \hline
  Classes & \texttt{CapitalizedWords} \\
 \hline
  Variables locales & \texttt{lowercase} ou \texttt{lowercase\_with\_underscores} \\
 \hline
  Variables globales & \texttt{UPPERCASE} ou \texttt{UPPERCASE\_WITH\_UNDERSCORES} \\
 \hline
\end{tabular}
...
\begin{frame}[fragile]{Conventions PEP257}
  \begin{block}{{\large Docstrings}}
    \medskip
...
\begin{frame}[fragile]{Package \texttt{pylint}}
  \begin{block}{{\large Analyse de code}}
    \medskip
...
\section{Organisation d'un projet et packaging}
...
\begin{frame}[fragile]{Qu'est ce qu'un package ?}
  ...
  \begin{block}{Un package python est }
    ...
    \begin{enumerate}
      ...
      \item \textcolor{red}{installable} en ligne de commande (via pip, uv ou poetry)
      \item un moyen d'utiliser les fonctionnalités dans \textcolor{red}{un autre package}
    \end{enumerate}

    Par exemple : SciPy, NumPy, Pandas, Matplotlib
  \end{block}
...
\begin{frame}[fragile]{Évolution des outils de packaging}
  ...
  \begin{block}{Approche traditionnelle (jusqu'en 2023)}
    \begin{itemize}
      ...
      \item Gestion manuelle des dépendances et versions
    \end{itemize}
  \end{block}
  ...
  \begin{block}{Approche moderne (2024-2025)}
    \begin{itemize}
      ...
      \item Gestion automatique d'environnements virtuels
      \item Fichiers de lock pour la reproductibilité et le suivi des dépendances
      \item Installation rapide
    \end{itemize}
  \end{block}
...
\begin{frame}[fragile]{uv : présentation}
  ...
  \begin{block}{Qu'est-ce que uv ?}
    \begin{itemize}
      \item Outil ultra-rapide écrit en Rust
      \item Alternative à pip, setuptools, virtualenv, poetry
...
\end{lstlisting}
  ...
  \begin{alertblock}{Note}
    \medskip
    PEP 517 et 518 ont standardisé \texttt{pyproject.toml}. L'utilisation de \texttt{setup.py} seul n'est plus recommandée depuis 2021.
  \end{alertblock}
...
\begin{frame}[fragile]{Qu'est-ce que ``vibe coding'' (rappel) ?}
...
\begin{frame}{Bonnes pratiques pour le vibe coding (rappel)}
...
\begin{frame}{Outils de vibe coding}

  {\large \textbf{Avant toute chose !}}

    On parle de vibe coding quand on utilise un logiciel basé sur un ou plusieurs LLM.

    Ce type de logiciel \textbf{ne doit pas être confondu} avec un chatbot, c'est un peu plus compliqué : agents IA, intéractions avec le système d'exploitation, recherche sur le web, etc.

  {\large \textbf{Solutions existantes :}}
  \begin{itemize}
    \item ChatGPT : le chatbot historique
    \item \href{https://www.claude.com/product/claude-code}{Claude Code}
    \item \href{https://github.com/features/copilot}{Github copilot}
    \item \href{https://cursor.com/}{Cursor}
  \end{itemize}
...
\begin{tabular}{|l|l|l|l|}
    \hline
    \textbf{Outil} & \textbf{Tarif} & \textbf{Intégration} & \textbf{Fonctionnement} \\
    \hline
    \hline
    GitHub         & Gratuit avec limitations & IDEs  & Complétion de code \\
    Copilot        & Abonnements              &       & Chatbot \\
    \hline
    Cursor & Essai gratuit & IDEs  & Intéractions avec le \\
           & Abonnements   &       & système d'exploitation (\textbf{agents}) \\
           &               &       & Création de projets complets \\
    \hline
    Claude    & Gratuit avec limitations & Ligne de   & Intéractions avec le \\
    Code      & Abonnements              & commande   & système d'exploitation (\textbf{agents}) \\
              &                          & IDEs       & Création de projets complets \\
    \hline
    ChatGPT & Gratuit avec limitations & Interface web & Génération de code \\
            & Abonnements              &               &  \\
    \hline
\end{tabular}
}
...
\begin{frame}{Bonnes pratiques}
  ...
  Quelques bonnes pratiques, parmi d'autres :
  \begin{itemize}
    \item<+-> Voir le projet comme tout projet de développement informatique : définir les tâches, développer, tester, itérer
    \item<+-> Utiliser les outils d'IA comme des assistants, pas des remplacçants
    \item<+-> Versionner le code généré (git everything and everywhere!)
    \item<+-> Documenter le code (ou le faire faire par l'IA)
    \item<+-> Tout tester ! Tout vérifier ! 
  \end{itemize}
  
\end{frame}

\begin{frame}{Quid de l'empreinte carbone de l'IA}

  Les modèles de langue sont très gourmands en ressources !

  Quelques recommandations :
  \begin{itemize}
    \item Utiliser l'IA avec mesure et de façon responsable.
    \item Limiter l'utilisation de l'IA à des tâches à valeur ajoutée
    \item Utiliser des petits modèles
    \item Utilisr des solutions locales
  \end{itemize}
  
  \href{https://bonpote.com/intelligence-artificielle-le-vrai-cout-environnemental-de-la-course-a-lia/}{\underline{Le vrai coût environmental de la course à l'IA}}

\end{frame}


% Considering the Carbon Footprint of LLMs:

% Use LLMs responsibly: Limit your use of LLMs to tasks that truly benefit from their capabilities.

% Choose smaller, more efficient models: Opt for LLMs with a lower computational footprint whenever possible.

% Fine-tune existing models: Instead of training new models from scratch, consider fine-tuning pre-trained models on your specific task. This can significantly reduce the environmental impact.

% Use cloud platforms with sustainable practices: Choose cloud providers that are committed to using renewable energy and reducing their carbon emissions.
